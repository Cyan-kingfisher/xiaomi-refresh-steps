...
import com.cyan.running.exception.ApiErrorException;
...
@Slf4j
@Service
public class HuamiShuaBuServiceImpl implements HuamiShuaBuService {
    ...
    @Override
    @Async
    public CompletableFuture<Boolean> mainHandler(String user, String pass, Integer step){
        String accessCode = getAccessCode(user, pass);
        Map<String, String> login = null;

        login = login(accessCode);
        log.info("HuaMiApi.mainHandler.login: success");

        String loginToken = login.get("login_token");
        String userId = login.get("user_id");
        String appToken = getAppToken(loginToken);

        updateStep(appToken, userId, step);
        log.info("HuaMiApi.mainHandler.brush: success");
    ...
    public Map<String, String> login(String accessCode) {
        try {
            ...
            log.info("HuaMiApi.login_token:"+login_token);
            map.put("login_token", login_token);
            log.info("HuaMiApi.login.user-id:"+user_id);
    ...
    public void updateStep(String appToken, String userId, int stepC) throws ApiErrorException{
        Date date = new Date();
